"""
Quexy — Excel Connector
Loads Excel (.xlsx) files into an in-memory SQLite database.
...
class ExcelConnector(DataSourceConnector):
    """Connects an Excel file by loading each sheet into an in-memory SQLite database."""
    ...
    def connect(self, file_path: str, **kwargs) -> bool:
        """
        Load an Excel file into an in-memory SQLite database.
        Each sheet becomes a separate table.
        ...
        try:
            ...
            for sheet_name, df in excel_data.items():
                ...
                self.sheet_names.append(table_name)
            ...
            raise ConnectionError(f"Failed to load Excel file: {str(e)}")
    ...
    def execute_query(self, sql: str) -> list[dict[str, Any]]:
        ...
        if not self.db_connection:
            raise ConnectionError("No active connection. Load an Excel file first.")
    ...
    def get_tables(self) -> list[str]:
        """Return list of tables (one per Excel sheet)."""
        if not self.db_connection:
            return []
        cursor = self.db_connection.cursor()
